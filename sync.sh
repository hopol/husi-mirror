#!/usr/bin/env bash
# 手动镜像 xchacha20-poly1305/husi 的 dev 分支。
set -euo pipefail
...
VERSION="$(sed -n 's/^VERSION_NAME[[:space:]]*=[[:space:]]*//p' upstream/husi.properties | head -n 1 || true)"
...
git add upstream
git commit -m "chore: mirror Husi source $VERSION ($SHORT_COMMIT)"

git tag -a "mirror-source-v$VERSION-$SHORT_COMMIT" -m "Mirror Husi source v$VERSION ($SHORT_COMMIT)"
git push origin HEAD
git push origin "mirror-source-v$VERSION-$SHORT_COMMIT"

echo "已同步 Husi $UPSTREAM_BRANCH：$UPSTREAM_COMMIT"
